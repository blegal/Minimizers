        ...
        const int level_2)
{
        ...
        while ((counterA != nElementsA) && (counterB != nElementsB) && ndst < (_oBuff_)) {
            ...
            if (v1 == v2) {
                ...
                if (density <= dense_threshold){ //encode colors w/ list of int
                    dbg_cnt ++;
                    if (dbg_cnt < 115) std::cerr << "(=) Merging sparse element #" << dbg_cnt << " with minimizer " << v1 << " (density: " << density << ")\n";
            ...
            ///////////
                ...
                if (density <= dense_threshold){ //encode colors w/ list of int
                    dbg_cnt ++;
                    if (dbg_cnt < 115) std::cerr << "(A) Merging sparse element #" << dbg_cnt << " with minimizer " << v1 << " (density: " << density << ")\n";
                    if (ndst_sparse + ((density+granularity-1) / granularity) >= _o_sparse_Buff_) {
                        fdst_sparse->write(dest_sparse, sizeof(uint64_t), ndst_sparse);
                        ndst_sparse = 0;
                    }
            ...
            ///////////
                ...
                if (density <= dense_threshold){ //encode colors w/ list of int
                    dbg_cnt ++;
                    if (dbg_cnt < 115) std::cerr << "(B) Merging sparse element #" << dbg_cnt << " with minimizer " << v2 << " (density: " << density << ")\n";
                    if (ndst_sparse + ((density+granularity-1) / granularity) >= _o_sparse_Buff_) {
                        fdst_sparse->write(dest_sparse, sizeof(uint64_t), ndst_sparse);
                        ndst_sparse = 0;
                    }
    ...
    if (nElementsA == 0) {
        ...
        while (true) {
            //if (tmp_cnt > 1000000) {
                //std::cerr << "tmp_cnt: " << tmp_cnt << "\n";
                //std::cerr << "counterA: " << counterA << " / nElementsA: " << nElementsA << "\n";
    ...
    std::cerr << "end of loop, writing if dest\n";
